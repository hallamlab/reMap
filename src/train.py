...
def __train(arg):
    ...
    if arg.evaluate:
        print('\n{0})- Evaluating reMap model...'.format(steps))
        steps = steps + 1
        ...
        print('\t>> Evaluate...')
        prob, score = model.score_and_predictive_distribution(X=X, y=y, bags_labels=bags_labels, bags_correlation=rho,
                                                              label_features=features, centroids=C,
                                                              ...
                                                              file_name=arg.model_name, result_path=arg.rspath)
        desc = "### Scores...\n"
        desc += "\t>> Average predictive score: {0:.4f}\n".format(prob)
        desc += "\t>> Average predictive score: {0:.4f}".format(prob)
        save_data(data=desc, file_name=arg.file_name + "_pred_score.txt", save_path=arg.rspath,
                  mode="w", w_string=True, tag="scores to labeling data")
    ...
    if arg.transform:
        ...
        y_Bag = model.transform(X=X, y=y, bags_labels=bags_labels, bags_correlation=rho, label_features=features,
                                ...
                                max_sampling=arg.max_sampling,
                                snapshot_history=arg.snapshot_history, decision_threshold=arg.decision_threshold,
                                batch_size=arg.batch, num_jobs=arg.num_jobs, file_name=arg.file_name,
...
def train(arg):
    try:
        if arg.preprocess_dataset or arg.train or arg.evaluate or arg.transform:
            actions = list()
            if arg.preprocess_dataset:
